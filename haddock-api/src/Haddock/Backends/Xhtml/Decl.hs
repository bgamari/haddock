...
ppDecl summ links (L loc decl) (mbDoc, fnArgsDoc) instances fixities subdocs splice unicode qual = case decl of
  ...
  SigD (ClassOpSig _ lnames lty) -> ppLFunSig summ links loc (mbDoc, fnArgsDoc) lnames
      ...
      +++ shortSubDecls False
          (
            ...
            [ ppFunSig summary links loc doc names (hsSigType typ)
                       [] splice unicode qual
              | L _ (ClassOpSig _ lnames typ) <- sigs
  ...
  where
    ...
    minimalBit = case [ s | L _ (MinimalSig _ (L _ s)) <- lsigs ] of
      ...
      And xs : _ | sort [getName n | L _ (Var (L _ n)) <- xs] ==
                   sort [getName n | L _ (ClassOpSig _ ns _) <- lsigs, L _ n <- ns]
        -> noHtml
